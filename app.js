...
/* ════════════════════════════════════════════════════════════════
   2. DB — Supabase + Cache CRUD Layer
   ════════════════════════════════════════════════════════════════ */
...
const DB = {
  getUsers()    { return DB_CACHE.users; },
  getProducts() { return DB_CACHE.products; },
  getMessages() { return DB_CACHE.messages; },
  getWishlist() { 
    const session = this.getSession();
    if (!session) return [];
    return DB_CACHE.wishlist.filter(w => w.user_id === session.id).map(w => w.product_id); 
  },
  getSession()  { return JSON.parse(localStorage.getItem(DB_KEYS.session)  || 'null'); },

  saveUsers(data)    { DB_CACHE.users = data; },
  saveProducts(data) { DB_CACHE.products = data; },
  saveMessages(data) { DB_CACHE.messages = data; },
  saveWishlist(data) { 
    const session = this.getSession();
    if (!session) return;
    
    const oldWishlist = DB_CACHE.wishlist.filter(w => w.user_id === session.id).map(w => w.product_id);
    const added = data.filter(id => !oldWishlist.includes(id));
    const removed = oldWishlist.filter(id => !data.includes(id));

    DB_CACHE.wishlist = DB_CACHE.wishlist.filter(w => w.user_id !== session.id);
    data.forEach(id => DB_CACHE.wishlist.push({ user_id: session.id, product_id: id }));

    added.forEach(id => supabase.from('tip_wishlist').insert([{ user_id: session.id, product_id: id }]).then());
    removed.forEach(id => supabase.from('tip_wishlist').delete().match({ user_id: session.id, product_id: id }).then());
  },
  ...
  createUser(name, email, password, course = '') {
    ...
    supabase.from('tip_users').insert([user]).then();
    return { user };
  },
  ...
  updateUser(id, updates) {
    ...
    if (updateObj.joinedAt) { updateObj.joined_at = updateObj.joinedAt; delete updateObj.joinedAt; }
    supabase.from('tip_users').update(updateObj).eq('id', id).then();
    return users[idx];
  },
  ...
  createProduct({ title, description, price, category, image, condition, sellerId, sellerName }) {
    ...
    supabase.from('tip_products').insert([product]).then();
  ...
  updateProduct(id, updates) {
    ...
    supabase.from('tip_products').update(updateObj).eq('id', id).then();
    return products[idx];
  },
  ...
  deleteProduct(id) {
    ...
    this.saveProducts(products);
    supabase.from('tip_products').delete().eq('id', id).then();
  },

  getProductById(id) { 
    const p = this.getProducts().find(p => p.id === id) || null; 
    if (!p) return null;
    return {
      ...p,
  ...
  getConversations(userId) {
    ...
    messages.forEach(msg => {
      ...
      const receiverName = msg.receiver_name || msg.receiverName;
      const createdAt = msg.created_at || msg.createdAt;

      if (senderId !== userId && receiverId !== userId) return;
      const otherId   = senderId === userId ? receiverId : senderId;
      const otherName = senderId === userId ? receiverName : senderName;
      if (!convMap[otherId]) convMap[otherId] = { userId: otherId, name: otherName, messages: [], lastAt: createdAt };
      convMap[otherId].messages.push({
        ...msg, senderId, receiverId, senderName, receiverName, createdAt
      });
      if (createdAt > convMap[otherId].lastAt) convMap[otherId].lastAt = createdAt;
    });
  ...
  getThread(userAId, userBId) {
    return this.getMessages().map(msg => ({
      ...
      receiverName: msg.receiver_name || msg.receiverName,
  ...
  sendMessage({ senderId, senderName, receiverId, receiverName, text }) {
    ...
    const msg = {
      ...
      text, created_at: new Date().toISOString(), read: false,
    };
    this.saveMessages([...messages, msg]);
    supabase.from('tip_messages').insert([msg]).then();
    return {
      ...msg,
      senderId, senderName, receiverId, receiverName, createdAt: msg.created_at
    };
  },
};
...
async function syncFromSupabase() {
  try {
    const [u, p, m, w] = await Promise.all([
      supabase.from('tip_users').select('*'),
      supabase.from('tip_products').select('*'),
      supabase.from('tip_messages').select('*'),
      supabase.from('tip_wishlist').select('*')
    ]);
    
    DB_CACHE.users = (u.data || []).map(row => ({
      ...row,
      joinedAt: row.joined_at
    }));
    DB_CACHE.products = (p.data || []).map(row => ({
      ...row,
      sellerId: row.seller_id,
      sellerName: row.seller_name,
      createdAt: row.created_at
    }));
    DB_CACHE.messages = (m.data || []).map(row => ({
      ...row,
...
function adminDeleteUser(userId) {
  ...
  showConfirm(
    ...
    () => {
      ...
      // Delete from Supabase (Cascade handles related products and messages)
      supabase.from('tip_users').delete().eq('id', userId).then();
...
async function init() {
  $('products-grid').innerHTML = `<div class="empty-state"><div class="empty-icon">⏳</div><h3>Connecting to Database…</h3></div>`;
